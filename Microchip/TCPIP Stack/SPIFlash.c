...
#include "Common_Files/HardwareProfile.h"
#include <peripheral_24F/spi.h>
...
void SPIFlashInit(void)
{
	...
	// Read Device ID code to determine supported device capabilities/instructions
	{
	    ...
	    for(i = 0; i < 6; i++)
	    {
			...
			while(SPI1_Tx_Buf_Full);  //wait till completion of transmission
			//Dummy = SPIFLASH_SSPBUF;
		    while(!SPI1_Rx_Buf_Full);  //wait till completion of reception
			Dummy = SPIFLASH_SSPBUF;
		}
		//Dummy = SPIFLASH_SSPBUF;
...
/*****************************************************************************
  ...
  Returns:
    None
  ***************************************************************************/
void SPIFlashReadArray(DWORD dwAddress, char *vData, WORD wLength)
{
    volatile BYTE Dummy;
    BYTE vSPIONSave;
    #if defined(__18CXX)
...
/*****************************************************************************
  ...
  Remarks:
    ...
    memory parts.
  ***************************************************************************/
void SPIFlashWriteArray(char* vData, WORD wLen)
{
    volatile BYTE Dummy;
    BYTE vSPIONSave;
    #if defined(__18CXX)
